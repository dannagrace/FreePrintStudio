...
SUBMISSION_PACKET_TIMEOUT_SECONDS="${FREEPRINTSTUDIO_SUBMISSION_PACKET_TIMEOUT_SECONDS:-540}"
...
run_static_checks() {
  ...
  env \
    -u RELEASE_ENV_PATH \
    -u MANUAL_RELEASE_VERIFICATION_PATH \
...
run_contact_readiness_report_generation() {
  printf '== App Review contact readiness report ==\n'
...
run_app_store_connect_readiness_report_generation() {
  printf '== App Store Connect readiness report ==\n'
...
run_public_pages_validation() {
  printf '== Public pages validation ==\n'
  run_with_timeout "$PUBLIC_PAGES_REPORT_TIMEOUT_SECONDS" Scripts/validate_public_pages.sh
}
...
run_metadata_preflight_validation() {
  printf '== Metadata upload preflight ==\n'
