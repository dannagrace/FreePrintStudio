...
check_contains "AppStore/review-guideline-audit.md" "Replace YOURTEAMID with the Apple Developer Team ID before running signing or archive commands." "Review audit must warn operators to replace Team ID placeholders"
...
check_contains "Config/release.env.example" "Replace YOURTEAMID with the Apple Developer Team ID before running signing or archive commands." "Release environment template must warn operators to replace Team ID placeholders"
...
check_contains "AppStore/release-inputs-worksheet.md" "Scripts/print_release_input_status.sh --strict" "Release input worksheet must tell operators to run strict release input status after installing templates"
check_contains "AppStore/release-inputs-worksheet.md" "APP_PRIVACY_SKIP_PUBLISH" "Release input worksheet must document the App Privacy skip-publish dry-run switch"
check_contains "AppStore/release-inputs-worksheet.md" "IPA_PATH" "Release input worksheet must document the manual TestFlight IPA path"
check_contains "AppStore/release-inputs-worksheet.md" "TESTFLIGHT_CHANGELOG" "Release input worksheet must document TestFlight changelog notes"
...
check_contains "AppStore/release-inputs-worksheet.md" "Replace YOURTEAMID with the Apple Developer Team ID before running signing or archive commands." "Release input worksheet must warn operators to replace Team ID placeholders"
check_contains "AppStore/release-inputs-worksheet.md" "Replace apple-id@example.com with the App Store Connect Apple ID before running Fastlane Apple ID commands." "Release input worksheet must warn operators to replace Fastlane Apple ID placeholders"
...
check_contains "Scripts/print_release_input_status.sh" "Replace YOURTEAMID with the Apple Developer Team ID before running signing or archive commands." "Release input status next commands must warn operators to replace Team ID placeholders"
check_contains "Scripts/print_release_input_status.sh" "Replace apple-id@example.com with the App Store Connect Apple ID before running Fastlane Apple ID commands." "Release input status next commands must warn operators to replace Fastlane Apple ID placeholders"
...
check_contains "fastlane/Fastfile" "APP_STORE_CONNECT_API_KEY_JSON issuer_id must be an App Store Connect issuer UUID" "Fastfile must validate API JSON issuer_id format before App Store Connect uploads"
check_contains "fastlane/Fastfile" "APP_STORE_CONNECT_API_KEY_JSON key_filepath does not exist at the configured path" "Fastfile must validate API JSON relative key_filepath before App Store Connect uploads"
check_contains "fastlane/Fastfile" "APP_STORE_CONNECT_API_KEY_JSON key_filepath does not look like an App Store Connect .p8 private key" "Fastfile must validate API JSON key file content before App Store Connect uploads"
...
check_contains "fastlane/Fastfile" "ASC_KEY_ID must be a 10-character App Store Connect API key id" "Fastfile must validate ASC_KEY_ID format before App Store Connect uploads"
check_contains "fastlane/Fastfile" "ASC_ISSUER_ID must be an App Store Connect issuer UUID" "Fastfile must validate ASC_ISSUER_ID format before App Store Connect uploads"
check_contains "fastlane/Fastfile" "ASC_KEY_PATH does not look like an App Store Connect .p8 private key" "Fastfile must validate ASC_KEY_PATH file content before App Store Connect uploads"
...
check_contains "fastlane/Fastfile" "validate_app_store_build_number!" "Fastfile submit lane must validate the selected App Store build number before final submission"
...
check_contains "Scripts/validate_fastlane_release_lanes.sh" "validate_app_store_build_number!" "Fastlane lane validation must check selected App Store build number gates"
...
check_contains "FreePrintStudio/Printing/PrintService.swift" "info.outputType = .general" "PrintService must use document printing so photo-paper defaults do not override the PDF workflow"
check_contains "FreePrintStudio/Printing/PrintService.swift" "showsPaperSelectionForLoadedPapers = true" "PrintService must expose loaded paper selection for exact-size verification"
check_contains "FreePrintStudio/Printing/PrintService.swift" "choosePaper paperList" "PrintService must choose the closest supported paper to the PDF page size"
check_contains "FreePrintStudio/Printing/PrintService.swift" "bestPaper(forPageSize: pageSize" "PrintService must match the printer paper to the selected app paper size"
...
App Store Connect	blocker	App Store Connect account holder	ASC_KEY_ID	Config/release.env	ASC_KEY_ID is missing	Configure App Store Connect credentials in untracked Config/release.env, then run the credential check.	Scripts/check_app_store_connect_credentials.sh
App Store Connect	blocker	App Store Connect account holder	ASC_ISSUER_ID	Config/release.env	ASC_ISSUER_ID is missing	Configure App Store Connect credentials in untracked Config/release.env, then run the credential check.	Scripts/check_app_store_connect_credentials.sh
App Store Connect	blocker	App Store Connect account holder	ASC_KEY_PATH	Config/release.env	ASC_KEY_PATH is missing	Configure App Store Connect credentials in untracked Config/release.env, then run the credential check.	Scripts/check_app_store_connect_credentials.sh
...
check_contains "README.md" "Scripts/install_private_release_input_templates.sh --source-dir build/private-release-input-templates --target-dir Config" "README must document the generated private input template installer"
check_not_contains "README.md" "Scripts/bootstrap_release_env.sh" "README must not send release operators through stale bootstrap_release_env"
...
check_contains "README.md" "Replace YOURTEAMID with the Apple Developer Team ID before running signing or archive commands." "README must warn operators to replace Team ID placeholders"
check_contains "README.md" "Replace apple-id@example.com with the App Store Connect Apple ID before running Fastlane Apple ID commands." "README must warn operators to replace Fastlane Apple ID placeholders"
...
check_contains "AppStore/release-checklist.md" "Scripts/install_private_release_input_templates.sh --source-dir build/private-release-input-templates --target-dir Config" "Release checklist must include the generated private input template installer"
check_contains "AppStore/release-checklist.md" "Fill local release environment values in the git-ignored \`Config/release.env\` installed from generated handoff templates" "Release checklist must use the generated private input template installer as the release.env setup path"
check_not_contains "AppStore/release-checklist.md" "Fill local release environment values from \`Config/release.env.example\`" "Release checklist must not send release operators through the tracked release.env example as the setup path"
...
check_contains "AppStore/release-checklist.md" "Replace apple-id@example.com with the App Store Connect Apple ID before running Fastlane Apple ID commands." "Release checklist must warn operators to replace Fastlane Apple ID placeholders"
...
check_contains "AppStore/release-inputs-worksheet.md" "Scripts/install_private_release_input_templates.sh --source-dir build/private-release-input-templates --target-dir Config" "Release input worksheet must install generated private input templates safely"
check_not_contains "AppStore/release-inputs-worksheet.md" "Scripts/bootstrap_release_inputs.sh" "Release input worksheet must not send operators through stale bootstrap_release_inputs"
check_contains "Scripts/generate_manual_release_evidence_form.sh" "Scripts/install_private_release_input_templates.sh --source-dir build/private-release-input-templates --target-dir Config" "Manual release evidence form must install generated private input templates safely"
check_not_contains "Scripts/generate_manual_release_evidence_form.sh" "Scripts/bootstrap_release_inputs.sh" "Manual release evidence form must not send operators through stale bootstrap_release_inputs"
check_contains "Scripts/generate_manual_release_readiness_report.sh" "Scripts/install_private_release_input_templates.sh --source-dir build/private-release-input-templates --target-dir Config" "Manual release readiness report must install generated private input templates safely"
check_not_contains "Scripts/generate_manual_release_readiness_report.sh" "Scripts/bootstrap_release_inputs.sh" "Manual release readiness report must not send operators through stale bootstrap_release_inputs"
...
check_contains "Scripts/prepare_app_store_submission_packet.sh" "Scripts/install_private_release_input_templates.sh --source-dir build/AppStoreSubmissionPacket/private-release-input-templates --target-dir Config" "Submission packet action items must include the safe generated private input installer"
...
check_contains "Scripts/prepare_app_store_submission_packet.sh" "Replace YOURTEAMID with the Apple Developer Team ID before running signing or archive commands." "Submission packet command order must warn operators to replace Team ID placeholders"
check_contains "Scripts/prepare_app_store_submission_packet.sh" "Replace apple-id@example.com with the App Store Connect Apple ID before running Fastlane Apple ID commands." "Submission packet command order must warn operators to replace Fastlane Apple ID placeholders"
...
check_contains "Scripts/generate_app_store_connect_readiness_report.sh" "Replace apple-id@example.com with the App Store Connect Apple ID before running Fastlane Apple ID commands." "App Store Connect readiness report must warn operators to replace Fastlane Apple ID placeholders"
...
App Store Connect	blocker	App Store Connect account holder	ASC_KEY_ID	Config/release.env	ASC_KEY_ID is missing	Configure App Store Connect credentials.	Scripts/check_app_store_connect_credentials.sh
App Store Connect	blocker	App Store Connect account holder	ASC_ISSUER_ID	Config/release.env	ASC_ISSUER_ID is missing	Configure App Store Connect credentials.	Scripts/check_app_store_connect_credentials.sh
App Store Connect	blocker	App Store Connect account holder	ASC_KEY_PATH	Config/release.env	ASC_KEY_PATH is missing	Configure App Store Connect credentials.	Scripts/check_app_store_connect_credentials.sh
EOF
...
check_contains "Scripts/prepare_app_store_submission_packet.sh" "Scripts/install_private_release_input_templates.sh --source-dir build/AppStoreSubmissionPacket/private-release-input-templates --target-dir Config" "Submission packet command order must install generated private input templates safely"
check_not_contains "Scripts/prepare_app_store_submission_packet.sh" "Scripts/bootstrap_release_inputs.sh" "Submission packet command order must not send handoff operators through stale bootstrap_release_inputs"
check_not_contains "Scripts/prepare_app_store_submission_packet.sh" "Scripts/bootstrap_release_env.sh" "Submission packet command order must not send handoff operators through stale bootstrap_release_env"
...
check_workflow_step_timeout "Review UI validation" "15" "Review UI workflow step must allow slow CI UITest Runner builds"
...
check_contains "Scripts/download_latest_submission_packet.sh" "FREEPRINTSTUDIO_ARTIFACT_DOWNLOAD_TIMEOUT_SECONDS" "Latest CI submission packet download helper must allow per-attempt timeout overrides"
check_contains "Scripts/download_latest_submission_packet.sh" "FREEPRINTSTUDIO_ARTIFACT_DOWNLOAD_METHOD" "Latest CI submission packet download helper must allow operators to force the GitHub artifact API path"
check_contains "Scripts/download_latest_submission_packet.sh" 'download_method="${FREEPRINTSTUDIO_ARTIFACT_DOWNLOAD_METHOD:-api}"' "Latest CI submission packet download helper must default to the GitHub artifact API path"
check_contains "Scripts/download_latest_submission_packet.sh" "api, gh, or auto; default api" "Latest CI submission packet download helper usage must document the API-first default"
...
check_contains "Scripts/download_latest_submission_packet.sh" "Artifact download command timed out" "Latest CI submission packet download helper must report download timeouts clearly"
check_contains "Scripts/download_latest_submission_packet.sh" 'run_with_timeout "$download_timeout_seconds" gh run download "$run_id"' "Latest CI submission packet download helper must bound each gh artifact download attempt"
check_contains "Scripts/download_latest_submission_packet.sh" "download_with_artifact_api" "Latest CI submission packet download helper must fall back to the GitHub artifact API when gh run download stalls"
check_contains "Scripts/download_latest_submission_packet.sh" 'actions/runs/$run_id/artifacts' "Latest CI submission packet download helper must discover the release artifact through the GitHub API"
check_contains "Scripts/download_latest_submission_packet.sh" 'actions/artifacts/$artifact_id/zip' "Latest CI submission packet download helper must download the release artifact archive through the GitHub API fallback"
check_contains "Scripts/download_latest_submission_packet.sh" "gh auth token" "Latest CI submission packet download helper must authenticate artifact API fallback downloads through GitHub CLI auth"
check_contains "Scripts/download_latest_submission_packet.sh" "curl --fail --location" "Latest CI submission packet download helper must follow GitHub artifact archive redirects with curl"
check_contains "Scripts/download_latest_submission_packet.sh" "zipfile.ZipFile" "Latest CI submission packet download helper must extract API-downloaded artifact zip files"
...
check_contains "README.md" "FREEPRINTSTUDIO_ARTIFACT_DOWNLOAD_TIMEOUT_SECONDS" "README must document the CI artifact download timeout override"
check_contains "README.md" "FREEPRINTSTUDIO_ARTIFACT_DOWNLOAD_METHOD=api" "README must document forcing the CI artifact API download path"
check_contains "README.md" "uses the GitHub artifact API download path by default" "README must document the API-first CI packet download default"
check_contains "README.md" "GitHub artifact API fallback" "README must document the CI packet download API fallback"
...
check_contains "AppStore/release-checklist.md" "FREEPRINTSTUDIO_ARTIFACT_DOWNLOAD_TIMEOUT_SECONDS" "Release checklist must document the CI artifact download timeout override"
check_contains "AppStore/release-checklist.md" "FREEPRINTSTUDIO_ARTIFACT_DOWNLOAD_METHOD=api" "Release checklist must document forcing the CI artifact API download path"
check_contains "AppStore/release-checklist.md" "uses the GitHub artifact API download path by default" "Release checklist must document the API-first CI packet download default"
check_contains "AppStore/release-checklist.md" "GitHub artifact API fallback" "Release checklist must document the CI packet download API fallback"
check_file "Scripts/validate_release_handoff_summary.sh" "Release handoff summary validator is required"
if [[ -f "Scripts/validate_release_handoff_summary.sh" && ! -x "Scripts/validate_release_handoff_summary.sh" ]]; then
  printf 'FAIL: Release handoff summary validator must be executable (Scripts/validate_release_handoff_summary.sh)\n'
  failures=$((failures + 1))
fi
...
EOF
  cat >"$handoff_local_readiness_path" <<'EOF'
BLOCKED: local blocker
Summary: 1 blocker(s), 0 warning(s).
EOF
  cat >"$handoff_actions_path" <<'EOF'
category	severity	owner	field	target	item	next_action	validation_command
Signing	blocker	Release owner	DEVELOPMENT_TEAM_ID	Config/release.env	Apple Developer Team ID missing	fill it	Scripts/check_code_signing_assets.sh
...
check_contains "Scripts/generate_release_input_todo.sh" "Scripts/print_release_input_status.sh --strict" "Release input TODO generator must tell operators to rerun strict redacted status after installing private templates"
...
EOF
  if ! Scripts/generate_release_input_todo.sh "$release_input_todo_actions" "$release_input_todo_output" >"$release_input_todo_test_dir/generate.log" 2>&1; then
    ...
    if ! grep -q 'run `Scripts/print_release_input_status.sh --strict` before filling release values' "$release_input_todo_output"; then
      printf 'FAIL: Release input TODO must rerun strict redacted status after release.env template install\n'
      failures=$((failures + 1))
    fi
    if grep -q 'create it from the private templates with `Scripts/bootstrap_release_inputs.sh` before filling release values' "$release_input_todo_output"; then
      printf 'FAIL: Release input TODO must not use stale release.env bootstrap guidance when generated private templates are available\n'
      failures=$((failures + 1))
    fi
    ...
    if ! grep -q 'run `Scripts/print_release_input_status.sh --strict` before recording evidence' "$release_input_todo_output"; then
      printf 'FAIL: Release input TODO must rerun strict redacted status after manual evidence template install\n'
      failures=$((failures + 1))
    fi
    if grep -q 'create it from the private templates with `Scripts/bootstrap_release_inputs.sh` before recording evidence' "$release_input_todo_output"; then
      printf 'FAIL: Release input TODO must not use stale manual evidence bootstrap guidance when generated private templates are available\n'
      failures=$((failures + 1))
    fi
...
check_contains "Scripts/validate_release_input_todo.sh" "Scripts/print_release_input_status.sh --strict" "Release input TODO validator must require strict redacted status guidance after private template install"
...
EOF
  if ! Scripts/generate_release_input_todo.sh "$release_input_todo_validator_actions" "$release_input_todo_validator_output" >"$release_input_todo_validator_test_dir/generate.log" 2>&1; then
    ...
    if Scripts/validate_release_input_todo.sh "$release_input_todo_validator_actions" "$release_input_todo_validator_bad" >"$release_input_todo_validator_test_dir/validate-missing-release-env-bootstrap.log" 2>&1; then
      ...
      failures=$((failures + 1))
    elif ! grep -q 'Config/release.env private template installer guidance' "$release_input_todo_validator_test_dir/validate-missing-release-env-bootstrap.log"; then
      printf 'FAIL: Release input TODO validator must identify missing release.env installer guidance\n'
      failures=$((failures + 1))
    fi
    cp "$release_input_todo_validator_output" "$release_input_todo_validator_bad"
    perl -0pi -e 's/^After installing or syncing templates, run `Scripts\/print_release_input_status\.sh --strict` before filling release values\.\n//m' "$release_input_todo_validator_bad"
    if Scripts/validate_release_input_todo.sh "$release_input_todo_validator_actions" "$release_input_todo_validator_bad" >"$release_input_todo_validator_test_dir/validate-missing-release-env-status.log" 2>&1; then
      printf 'FAIL: Release input TODO validator must reject missing release.env strict status guidance\n'
      failures=$((failures + 1))
    elif ! grep -q 'Config/release.env strict release input status guidance' "$release_input_todo_validator_test_dir/validate-missing-release-env-status.log"; then
      printf 'FAIL: Release input TODO validator must identify missing release.env strict status guidance\n'
      failures=$((failures + 1))
    fi
    cp "$release_input_todo_validator_output" "$release_input_todo_validator_bad"
    ...
    if Scripts/validate_release_input_todo.sh "$release_input_todo_validator_actions" "$release_input_todo_validator_bad" >"$release_input_todo_validator_test_dir/validate-missing-manual-bootstrap.log" 2>&1; then
      ...
      failures=$((failures + 1))
    elif ! grep -q 'Config/manual-release-verification.env private template installer guidance' "$release_input_todo_validator_test_dir/validate-missing-manual-bootstrap.log"; then
      printf 'FAIL: Release input TODO validator must identify missing manual evidence installer guidance\n'
      failures=$((failures + 1))
    fi
    cp "$release_input_todo_validator_output" "$release_input_todo_validator_bad"
    perl -0pi -e 's/^After installing or syncing templates, run `Scripts\/print_release_input_status\.sh --strict` before recording evidence\.\n//m' "$release_input_todo_validator_bad"
    if Scripts/validate_release_input_todo.sh "$release_input_todo_validator_actions" "$release_input_todo_validator_bad" >"$release_input_todo_validator_test_dir/validate-missing-manual-status.log" 2>&1; then
      printf 'FAIL: Release input TODO validator must reject missing manual evidence strict status guidance\n'
      failures=$((failures + 1))
    elif ! grep -q 'Config/manual-release-verification.env strict release input status guidance' "$release_input_todo_validator_test_dir/validate-missing-manual-status.log"; then
      printf 'FAIL: Release input TODO validator must identify missing manual evidence strict status guidance\n'
      failures=$((failures + 1))
    fi
    cp "$release_input_todo_validator_output" "$release_input_todo_validator_bad"
    ...
    cp "$release_input_todo_validator_output" "$release_input_todo_validator_bad"
    perl -0pi -e 's/(\| blocker \| QA\/release owner \| `MANUAL_REAL_IPHONE_MODEL` \| Real iPhone model is missing \| Record real iPhone evidence\. \| )`APP_STORE_BUILD_NUMBER=PROCESSED_BUILD_NUMBER Scripts\/validate_manual_release_verification\.sh`/$1`Scripts\/validate_app_review_contact.sh`/' "$release_input_todo_validator_bad"
    if Scripts/validate_release_input_todo.sh "$release_input_todo_validator_actions" "$release_input_todo_validator_bad" >"$release_input_todo_validator_test_dir/validate-mismatched-row.log" 2>&1; then
      printf 'FAIL: Release input TODO validator must reject action detail rows with mismatched validation commands\n'
      failures=$((failures + 1))
...
EOF
  if ! Scripts/generate_release_phase_plan.sh "$phase_plan_actions" "$phase_plan_output" >"$phase_plan_test_dir/generate.log" 2>&1; then
    ...
    if ! grep -q 'Required final submission guards' "$phase_plan_output"; then
      printf 'FAIL: Release phase plan must include required final submission guards\n'
      failures=$((failures + 1))
    fi
    if ! grep -q '| `APP_STORE_BUILD_NUMBER` | Processed App Store Connect build selected for App Review. |' "$phase_plan_output"; then
      printf 'FAIL: Release phase plan must include the selected App Store build guard\n'
      failures=$((failures + 1))
    fi
    ...
    phase_plan_bad="$phase_plan_test_dir/release-phase-plan-missing-final-guard.md"
    cp "$phase_plan_output" "$phase_plan_bad"
    perl -0pi -e 's/^\| `CONFIRM_SUBMIT_FOR_REVIEW=1` .*\n//m' "$phase_plan_bad"
    if Scripts/validate_release_phase_plan.sh "$phase_plan_actions" "$phase_plan_bad" >"$phase_plan_log" 2>&1; then
      printf 'FAIL: Release phase plan validator must reject missing final submit guard guidance\n'
      failures=$((failures + 1))
...
check_contains "Scripts/preflight_release_handoff.sh" 'Scripts/validate_release_handoff_summary.sh "$summary_path"' "Release handoff preflight must validate generated handoff summaries"
...
check_contains "Scripts/preflight_release_handoff.sh" "external-readiness-actions.tsv" "Release handoff preflight must surface the external readiness action manifest"
check_contains "Scripts/preflight_release_handoff.sh" 'Scripts/validate_external_readiness_actions.sh "$packet_dir/readiness.txt" "$external_actions_path"' "Release handoff preflight must validate CI external readiness actions before handoff"
...
check_contains "FreePrintStudio.xcodeproj/project.pbxproj" "CURRENT_PROJECT_VERSION = 2" "Build number must be set"
