...
def check_char_limit(label: str, path: str, minimum: int | None, maximum: int) -> None:
    text = read_text(METADATA / path)
...
check_char_limit("Review notes", "review_information/notes.txt", None, 4000)

copyright_text = read_text(METADATA / "copyright.txt")
